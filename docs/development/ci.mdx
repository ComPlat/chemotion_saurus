...
The Chemotion ELN's CI runs with [GitHub Actions](https://github.com/features/actions), an automation tool by GitHub.
It automatically runs the test suite on pushes to the
...
The CI is configured in the following [workflows](https://github.com/ComPlat/chemotion_ELN/tree/main/.github/workflows).
...
The Docker image for CI is hosted [here](https://hub.docker.com/r/complat/complat-ubuntu-runner).
...
The CI uses a [PostgreSQL container](https://hub.docker.com/_/postgres) to run the database alongside the ELN application.
