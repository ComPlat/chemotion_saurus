...
One nice thing about Cypress API Testing is that you can conveniently work with the response data right from the test Runner. 
When you click on the request step, Cypress will display the corresponding response data in the browser's console.

Consider following example, where we are testing 'Collection API (GET request)'
...
```
Here, were are sending http GET request to a 'api/v1/collections/roots.json' endpoint and checking its response.status field to be matched with code '200'. The response body can contain many objects, arrays, attributes, etc. This is how we can access a specific component from its body and perform some checks on it.
...
// check if returned fields are matching exactly what t be expected
expect(collectionData.id).to.eq(3);
expect(collectionData.label).to.eq('Col1');
```
