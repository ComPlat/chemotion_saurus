...
The Chemotion ELN is tested using three different kinds of tests.
...
Here, we are sending a GET request to the `api/v1/collections/roots.json` endpoint and verify that its `response.status` is `200`.
...
When testing a component which includes a store you need to explicitly import the store in the test file. The import must be before the import statement of the component to test.
...
The `expect` extension by mocha refers to the current test-object by `this`. Using a lambda expression at the it definition, mocha has no access to it. Using an explicit
...
- for more info: [rspec-mocks](https://rspec.info/features/3-13/rspec-mocks/)
