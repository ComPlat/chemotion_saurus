...
This chapter documents the Chemotion team's git-workflow in order to reduce uncertainty and ambiguity in collaboration.

The team uses a centralized git-workflow with feature branches.
The workflow is centralized in the sense that internal team members have write access to the
online [repository](https://github.com/ComPlat/chemotion_ELN),
and contribute directly to the online repository, rather than contributing via forks.
...
The following illustrates the git-workflow by showing the life-cycle of a feature branch `feature`:
from creating the branch, to opening a pull request, to eventually merging it into the default branch `main`.

The illustration starts with a visual representation of the git-workflow,
...
![diagram_git_workflow](/img/development/git/git-workflow.drawio.svg)
...
On this project, the team merges branches without separate merge commits.
That means the team integrates changes by **rebasing** in order to maintain a clean git history.
...
The team merges PRs by squashing all commits on the feature branch,
...
Consequently, the team can conveniently trace which features have been merged into `main` and in which order.
The merge-commit message by default includes the PR number,
so that the full history of a PR can be accessed conveniently (despite it being squashed).
