...
[Contact us](https://chemotion.net/helpdesk) if you're having trouble setting up your development environment.
...
Execute the following commands from the root of the repository (i.e., `pwd` is `chemotion_ELN`).
Start by preparing some configuration files
...
To run Chemotion via Docker, we need three containers:
...
We first build and prepare the "app" container via

```shell
gid=$(id -g) uid=$(id -u) docker compose -f docker-compose.dev.yml --env-file .dockerenv build setup
```

Now we can instantiate the development environment with

```shell
docker compose -f docker-compose.dev.yml --env-file .dockerenv up
```

The first time this command is executed may take a while before Chemotion is accessible from your browser,
even if the "app" container has already started, since the "webpacker" container only starts building once the
"app" container is up.

Docker will give each of the three containers a unique name that you can query with
...
For more context visit its [documentation](https://code.visualstudio.com/docs/devcontainers/containers).

### Dependencies

#### Visual Studio Code

Install [Visual Studio Code](https://code.visualstudio.com) (referred to as `VSCode` in the following) on your operating system.
...
The VSCode IDE is now running on your host operating system.
