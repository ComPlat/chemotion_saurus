---
id: development
title: Development of Chemotion ELN
...
The Chemotion team recommends setting up a development environment in a Visual Studio Code devcontainer.
You can find more information [here](development/environment).

## Learning resources

To learn more about the technologies used on the project, have a look at the following
[resources](development/learn).
...
For example, the JavaScript files with all React components formerly lived in `app/assets/javascripts` and have now moved to `app/packs/src`.
`app/packs/src` contains a directory for each single-page application (SPA). For example, the user-facing SPA lives in `app/packs/src/components`.
...
For instance, in the **devise** subdirectory, a [package](https://github.com/heartcombo/devise) for authentication is configured, which takes care of the mail sender and passwords.
...
Contains Ruby and JavaScript unit tests, as well as feature/acceptance tests.
...
Visit the [linting chapter](development/linting) for more details.
