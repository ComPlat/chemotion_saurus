...
This chapter follows the example of investigating the performance of creating a reaction.
It only deals with the backend for the moment, meaning the performance investigation will be limited to Ruby.

For a good general primer on performance investigations, have a look at this video:

<YouTubeFrame src="https://www.youtube-nocookie.com/embed/XL51vf-XBTs" />
...
You can quantify performance by using a benchmarking tool.
...
You're now ready to include `rspec-benchmark` syntax / DSL in your test (for details head over to the `rspec-benchmark` docs):
...
In order to improve performance, we now need to identify hot spots in the code that lend themselves to optimization.
...
`stackprof` can also break down how much time the test spends on each line of a function call.
