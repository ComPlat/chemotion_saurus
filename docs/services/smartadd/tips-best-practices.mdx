...
The following practices work best for SmartAdd. These tips save users time and headaches.

## Before You Start

### Name Your Files Well

Because users build the folder structure manually in SmartAdd, clear file names are essential.

**What makes a good file name?**
- Include the [sample](/docs/eln/ui/elements/samples) or compound identifier
...
Before uploading, collect all the necessary files:
...
:::tip[Upload Strategy]
Whether uploading a ZIP or individual files, SmartAdd puts everything in an input list. Users then manually drag files to assign them to samples and analyses.
:::
...
The Chemotion team recommends completing each sample fully before starting the next one:
...
- Less overwhelming with lots of data
- Users won't forget to assign files

:::tip[Don't Forget!]
Drag files from the left panel (input list) to the right panel (assignment tree). If the user does not assign a file, it will not be in the export!
:::

### Keep Names Consistent

Pick a naming style and stick with it throughout the project:
...
- Type in the molecular structure using SMILES notation
- Look for the green indicator (valid) or red (invalid)
- Fix any errors before moving on
- Not sure about the SMILES? Use [ketcherDemo](https://lifescience.opensource.epam.com/KetcherDemoSA/index.html) to generate a valid molfile

### Group Samples into Reactions

For a [reaction](/docs/eln/ui/elements/reactions) with multiple samples:
...
Descriptive entries help when revisiting the data later:
...
Formatted text makes the data more readable and professional!
:::

### Master the Two-Panel Layout

**Left Panel (Input List):**
- This is where all uploaded files live
- Drag files from here to assign them to samples

**Right Panel (Assignment Tree):**
- This is the workspace with samples and reactions
- Drag files here to attach them
- Click on any item to edit its properties

**Pro tip:** Expand the section currently in progress, and collapse the finished ones. Keeps things tidy!
...
Before clicking "Download as ZIP", run through this:
...
- [ ] Status updated
- [ ] Analyses reordered to the preferred sequence (drag-and-drop)
...
The **Type (Ontology)** dropdown for analyses (CHMO) and reactions (RXNO) keeps a history of recent picks:
- Open the dropdown — the last used terms appear at the top under **"-- Recently selected --"**
...
Put the primary analyses (e.g. 1H NMR) first so reviewers see the most important data at the top.
...
- Hover the mouse over any reaction in the tree
...
**Import from [Chemscanner](/docs/services/chemscanner) to create collection structure:**
