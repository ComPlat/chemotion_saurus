...
This guide walks users through the entire workflow from uploading data to importing it into Chemotion.

## Step 1: Upload Analytical Data Files

### Upload Your Analytical Files

Upload the analytical data files to SmartAdd. These files will be stored in the input list, ready for the user to assign to [samples](/docs/eln/ui/elements/samples) and analyses.
...
Options A and B are for uploading the **analytical data files** (spectra, images, raw data):
- Files appear in the **input list (left panel)**
- Users manually create the collection structure (samples, reactions)
...
Instead of manually creating the collection structure, users can automatically generate it from:

### Option C: Import from Chemscanner Export (export.json)

Use this to automatically create samples and [reactions](/docs/eln/ui/elements/reactions) from [Chemscanner](/docs/services/chemscanner) data:
...
This option creates the **collection structure** automatically - there is no need to manually create samples and reactions. The structure is ready to use; just assign the analytical data files to it!
:::
...
Perfect for batch processing multiple samples! Use the provided template format to structure the data in Excel, then import it to SmartAdd. The entire hierarchy is generated automatically - just assign the analytical files afterwards.
:::
...
**Options C and D create the collection tree structure automatically:**
...
Now users create the structure by manually adding samples and reactions, then assigning the uploaded files to them.
...
There are multiple ways to build and organize the collection:
...
Choose the method that feels most natural. The context menu (right-click) is especially useful for quickly adding nested items without navigating through menus.
:::
...
- **SMILES**: Syntax is checked automatically as the user types
...
The **Type (Ontology)** dropdown (CHMO for analysis type, RXNO for reaction type) remembers the user's recent selections:
...
Users now have two export options:
...
2. **Enter Server Details**:
   - Chemotion server URL (e.g., https://chemotion.dept.uni.de)
...
1. Open the Chemotion ELN
2. Navigate to the collections
3. The data is already there after a few seconds of upload - ready to use!
