...
SmartAdd doesn't automatically organize the analytical files. After uploading:
1. Look in the **input list** on the left - your files are there
2. Create [samples](/docs/eln/ui/elements/samples) and [reactions](/docs/eln/ui/elements/reactions) using the buttons in the right panel
3. **Drag analytical files** from the left input list to samples/analyses in the right tree

:::warning
Files that stay in the input list won't be exported! Make sure to drag everything needed to the assignment tree.
:::

**For Chemscanner ZIP and Excel Templates:**
These work differently - they **auto-generate the collection structure**:
- [Chemscanner](/docs/services/chemscanner) ZIPs (with `export.json`) create samples and reactions automatically
- Excel templates generate the complete collection hierarchy from the spreadsheet
...
- Users can edit and refine the auto-generated structure as needed

### Chemscanner ZIP not auto-generating structure

If the Chemscanner ZIP uploads but doesn't create samples/reactions:
...
**Workaround:**
If auto-processing fails, users can still work manually:
...
If the Excel file uploads but doesn't create samples/reactions:
- **Using the official template?** Users must use the SmartAdd Excel template provided by SmartAdd
...
Make sure you're clicking on an actual item (like a sample or reaction), not a folder. The properties panel should appear on the right when the user selects something.
...
This is fixed in the latest version! If the problem persists:
...
The Quill editor loads dynamically. If a gray loading box never loads:
- Refresh your browser (`Ctrl+F5` or `Cmd+Shift+R`)
...
If the uploaded MOL file shows validation errors:
- **Check the error message**: It indicates exactly what's wrong
- **Common issues**:
...
If the rich text formatting isn't saved:
...
- The collection might be too large - try breaking it into smaller batches
