...
The creator of a reader mostly requires knowledge in (e.g. Python) coding, while the admin of the second layer has to know the measurement technique and device very well, because it decides which info the user will get and see at the end of the third layer.
...
When using the standalone version, the user just writes the "converter-URL"/admin/ into the browser's address bar.
And when using the ELN integrated version, the user has to be logged in as an admin and click on the "Converter Profiles"-Tab on the left side.
...
After clicking on the "Create new new profile"-Button, the user can finally start to ...
