...
:::warning[warning]
Note that drag-n-dropping the orignal file only from the inbox into an anlysis will not trigger the converter if the converter already has processed it and deposited the resulting zip along the original.
