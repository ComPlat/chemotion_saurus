...
Data Type contains a drop-down list of some ontologies (mostly chemical analytic techniques) defining the kind of layout for displaying and plotting curves and spectra in ChemSpectra.
Not all ontologies known in the ELN are findable here because it's only for the plotting layout purpose.
If you think that something is missing, feel free to tell us.

##### X & Y Units

Mostly self-explanatory but it contains a list of physical units.
The most common units with and without prefixes are inside the lists but if you need others, please tell us or use the "scalar operation" feature to convert the column values.

##### Table columns

Also mostly self-explanatory.
It's a drop-down menu for defining the x and y column values while a preview can be seen on the end of the left side as "Input table data".
...
Alternative you can use the "column operation" feature to add, subtract, divide or multiply multiple columns together.

In some futur version it will become possible to create profiles without Output tables if you intend to use the converter only for Metadata extraction like descripted down below. 
