...
The **Copy** function enables users to create a duplicate of an Element for their content. Once users click on the "Copy" button and select the [Collection](/docs/eln/ui/collections) to assign the new Element to, the data is copied to the new Element. Users can then edit the Element name and other details as needed. This enables them to efficiently replicate complex content without starting from scratch.
