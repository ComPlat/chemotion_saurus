...
This guide explains how to add, view, and manage funding information for [Reactions](/docs/eln/ui/elements/reactions), [Samples](/docs/eln/ui/elements/samples), and [Embargoed](/docs/repo/workflow/embargo) data in the [Chemotion Repository](/docs/repo).

---

## What are Funding References?

Funding references allow users to document which organizations or grants supported their research. This information is essential for:
...
Users can attach funding references to the following data entities:
...
If a Reaction or Sample needs its own specific funding reference, users can add or remove it individually - separately from the embargo.

**Embargo-level funding:**
When a user adds a funding reference to an embargo, it will automatically apply to all the Reactions and Samples under that embargo. This means the user does not need to add the same funding to each item manually. If the user later removes the funding reference from the embargo, it will also be removed from all the associated Reactions and Samples.
...
### 2. View Existing Funding
  In the **Funding References** window, all existing funding entries are listed - whether the user is viewing a Reaction, Sample, or Embargo. Use this window to review the current funder and award information already linked to the item.
...
### 3. Add New Funding
  ...
  - **Search and Select:**
    ...
    - If the user selects "Other", they will need to manually enter the organization details.
  - **Fill in Details**:
  ...
  - **Save**:
    - Click the <MdxButton variant="success">Add Funding Reference</MdxButton> button to save the funding entry. It will appear in the funding table associated with the user's item.
...
- Users will see the funder name, type (Crossref, ROR, or Other), identifier, and any award information.
...
- Users may add **multiple funding references** to a single entity, if supported by more than one funder.
- If users have questions or encounter issues, please contact the Chemotion team at chemotion-repository@lists.kit.edu.
