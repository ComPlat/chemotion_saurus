...
| mass       | Advion Expression-L (CN)| .datx (.cdf)        |   Yes       |  aut. conversion to .cdf               |
| IR         | Bruker Alpha (CS, CN)   | .dx                 |   Yes       |                 |
| Raman      | Bruker Multiram (CN)    | .0 (.dx (jcamp)     |   tbd          |                 |
| UV-Vis     | Horiba (CN)             | .txt                |   Yes       |                 |
| UV-Vis     | xxxxxx (CS)             | xxx                 |   tbd       |                 |
| EA         | EA (CS)                 | .png                |   No        |                 |
| X-ray      | all                     | .cif + png          |   No        |                 |
| GC-FID/MS  |Shimadzu GC/MS (CN)      | .gcd (.txt (ASCII Format); .cdf (AIA(Andi)-Format))|   No      |
| GC-BID     |Shimadzu GC-BID 2030 (CN)| .gcd (.txt (ASCII Format); .cdf (AIA(Andi)-Format))|   No        |                  |
| TGA        | TA Instruments (CN)     | .tri (.xls)         |   No        |  manual conversion into .xls  |
| LC/MS      | Agilent IQ (CN)         | .sirstl folder (.cdf und .ms (AIA-Format) und .dx (CDS2 raw-Format)) |   No     |
| Polarimeter| Anton Paar MCP4100 (CN) | .pdf                |   No        |                             |
| DSC        | TA Instruments (CN/SML) | .tri (.xls)         |   No        | manual conversion into .xls |          
