---
title: DSC
sidebar_label: DSC
author: Nicole Jung
sidebar_position: 16
---

:::info[Important information for this page] 

This page contains a collection of information how to reference analytical data. We provide this page to describe the preferred formatting of analytical evaluations in the Chemotion repository. The given preferences follow standards of journals/publishers and other stakeholders wherever we could find them. If you have suggestions and comments: please let us know or provide your ideas via the issue tracking option. 
https://github.com/ComPlat/chemotion_saurus/issues
Updated: 06.11.2024
:::

## Inline notation for TGA thermal gravimetric analysis (DSC) 

The inline notation gives a short summary of the metadata and a list of the results of the DSC measurement. 
The analysis of the DSC measurements should report values from all segment. To be reported are the glass transition temperature (Tg) (maximum/minimum of first derivative), the melting temperature (Tm) (null of first derivative).
inlcuding the melting enthalpy (integral of melting peak). 
System peaks need to be omitted (by telling the software to cut the analysis temperature by  ~20 °C on every side)

### Examples for desired formatting:

**Standard example:** <br /> 
DSC (150 to -150, 10 °C/min): Temp. in °C (heat flow in mW/mg, Tx, Seg) = 50 (T<sub>g</sub>, S2), 100 (area, T<sub>m</sub>, S2), 55 (T<sub>g</sub>, S4), 102 (area, T<sub>m</sub>, S4). <br />







