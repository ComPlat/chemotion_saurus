...
as well as in the [this Documentation](../documentation#links) chapter.
...
Find more information about presets [here](https://docusaurus.io/docs/using-plugins#using-presets).
...
A VCS is also needed to enable [online editing](maintaining#contribution-to-online-version-via-version-control-system) and online Continuous Integration/Deployment.
...
There are two scenarios: 1. you are the creator and maybe admin of a Docusaurus instance, or 2. you are contributing to an existing Docusaurus instance.
...
Check if your Docusaurus instance could be built correctly. This command is also helpful before you push code in order to solve errors and detect incorrectly resolved links:
...
A static Docusaurus website can be built with an npm or yarn command, after which only this `build` directory needs to be pushed to a server:
...
built via npm or yarn and the resulting build directory pushed to a server via SSH.
