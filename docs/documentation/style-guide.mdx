...
- Within the same docs instance (ELN, Repository, or LabIMotion), prefer a relative path (`[samples](elements/samples)`) over an absolute `/docs/...` one -- a relative link stays correct regardless of which version prefix (e.g. `/next`, `/v2`) the current page is served under, while an absolute path always resolves to the *current* version even from an older one. Reserve an absolute `/docs/<instance>/...` path for links that cross into a *different* docs instance (e.g. a LabIMotion page linking to an ELN page), where a relative path can't reach.
