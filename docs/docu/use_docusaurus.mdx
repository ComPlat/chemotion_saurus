...
For changing the documentation you need a GitHub account which allows you to use the `Edit this page` button at the bottom left of every Docusaurus page.
Alternatively, you can start editing Markdown files (.md or .mdx file format) directly on GitHub by clicking the pencil icon on the top right.
This leads you to an editor on GitHub where you can directly edit the page and make a pull request or commit your changes directly, if you have write access.
For advanced contributions to the Docusaurus, you can clone the repository and push to it if you have write access (follow the instructions in the [set up chapter](Set%20Up#development)).
...
More information about GitHub Actions (in general and for the Chemotion ELN) can be found in the [testing chapter](development/testing_code).
...
### Info Boxes
Highlighted info boxes ([like on the eln intro page](intro)) with further information are called `Admonitions` in [Docusaurus](https://docusaurus.io/docs/markdown-features/admonitions).
