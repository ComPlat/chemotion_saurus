---
title: First Steps
sidebar_position: 31
---

import { faCog, faPlus, faSearch } from "@fortawesome/free-solid-svg-icons";
import { Btn } from "@site/src/js/btn.js";

## Registration

Registration is a necessary step towards your own laboratory notebook. Please register on your first visit to the electronic laboratory notebook. Then use this access data for each subsequent login. The registration page or the login page will always be the first page accessed.

## SciFinder/SciFinder<sup>n</sup> Credential

This step is only required if you have been granted permission to use the [Search CAS](../ui/details_modal#search-cas) (<Btn mixed={[faSearch, "Search CAS"]} color={"secondary"} />) feature (ELN 1.3+ versions) or the Scifinder tab feature (prior to ELN 1.0.0).
Both of these features query information in SciFinder/SciFinder<sup>n</sup> by using an API.

:::info NOTE
Please note that you must register once with CAS to obtain your personal CAS username and password.
:::

You can register the SciFindern credential in the "Account & Profile".

<img
  src={require("@site/static/img/scifindern_profile.png").default}
  alt={"SciFinderN Profile"}
/>

### ELN 1.3 and above (Search CAS)

:::info NOTE
This function is not accessible to all users, please ask your instance administrator if you don't see this function.
:::

<img
  src={require("@site/static/img/scifindern_function.png").default}
  alt={"CAS Search"}
/>

When you first see the SciFinder-n Credential, the expiry date is blank.

<img
  src={require("@site/static/img/scifindern_no_credential.png").default}
  alt={"SciFinderN No Credential"}
/>

After pressing the <Btn mixed={["Get token"]} color={"primary"} /> button, you will be redirected to the CAS login page. Please enter your CAS username and password to request an access token.

<img
  src={require("@site/static/img/scifindern_cas_login.png").default}
  alt={"SciFinderN CAS Login"}
/>

If you see a valid expiry date, you are ready to use the Search CAS feature. Please see [Search CAS](../ui/details_modal#search-cas).

<img
  src={require("@site/static/img/scifindern_has_credential.png").default}
  alt={"SciFinderN Credential"}
/>

### Prior to ELN 1.0.0 (SciFinder tab)

<img
  src={require("@site/static/img/scifinder_tab.png").default}
  alt={"SciFinder Tab"}
/>

If you're using a version of ELN prior to 1.0.0, you can register the SciFinder credential in the "Account & Profile".

![SciFinder Register](/img/scifinder_register.gif)

After pressing the <Btn mixed={["Submit"]} color={"info"}/> button, a token will show on the page if the SciFinder credential is valid.

![SciFinder Token](/img/scifinder_token.png)

If the credential is not vaild, "none" will be shown in the column "current token".

![SciFinder No_Token](/img/scifinder_no_token.png)

## The First Collection

![Collection View](/img/first_collection.gif)

A <b>collection</b> is a collection of elements (<b>Sample</b>, <b>Reaction</b>, <b>Wellplate</b>, <b>Screen</b>, <b>Research Plan</b> and others) that is assigned to a project or subproject. Two collections are given per default: <b>chemotion-repository.net</b> and <b>All</b>. The <b>chemotion-repository.net</b> collection is a connector to the chemotion repository and should not be used in the first Chemotion ELN trials. The collection <b>All</b> is installed as backup, it contains all entries of the Chemotion ELN independent of other assignments. The creation of the first own collection is usually the first step in the Chemotion ELN and allows to store your own elements.
For the creation of collections please use the collection management via the red button <Btn mixed={[faCog]} color={"danger"}/> in the collection bar. Please create a new entry <b>New Collection</b> under <b>My Collections</b> by pressing <Btn mixed={[faPlus]} color={"success"}/>. Please rename the collection and save it by pressing <Btn mixed={["Update"]} color={"warning"}/>.

:::danger Caution
To use the Chemotion ELN functions, a first user-defined collection has to be created!
Please think about a smart structure for your Chemotion ELN and create your first collection!
:::

## Chemotion ELN user settings

### User Settings: Account & Profile

The account & profile settings allow the user to

- define a reaction label and adjust the counter for reactions
- change the visibility of labels/numbers for samples
- choose a curation standard for the analytical section
- [add SciFinder/SciFinder<sup>n</sup> credentials](#scifinderscifindern-credential)
- adjust the counter for different elements in the Chemotion ELN
- set up his/her default structure editor

### User Settings: Template management

This function is not accessible to all users, please ask your instance admin if you don't see this function

### User Settings: Change Password

### User Settings: My affiliations

### User Settings: My groups

### User Settings: Molecule Moderator

This function is not accessible to all users, please ask your instance admin if you don't see this function

### User Settings: My devices

This function is not accessible to all users, please ask your instance admin if you don't see this function
