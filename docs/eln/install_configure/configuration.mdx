...
The default structure editor for Chemotion ELN is Ketcher version 1. It is seamlessly integrated through the ketcher-rails project and therefore no additional configuration is needed.

### Supported editors

The following editors are currently supported:

| Name        | ELN Versions | Compatible Editor Versions                                                                                           |
| ----------- | ------------ | -------------------------------------------------------------------------------------------------------------------- |
| ChemDraw JS | >= 1.0       | versions 17 to 22                                                                                                    |
| Ketcher 2   | < 1.9        | [2.4.2](https://github.com/epam/ketcher/releases/download/v2.4.2/ketcher-standalone-2.4.2.zip)                       |
| Ketcher 2   | >= 1.9       | versions 2.4.2 and [2.18.0](https://github.com/epam/ketcher/releases/download/v2.18.0/ketcher-standalone-2.18.0.zip) |
| Marvin JS   | >= 1.0       | 21.8                                                                                                                 |

If you wish to utilize these, additional configuration is required in the `structure_editors.yml` file following the steps described [above](#optional-modify-configuration-files). You will also have to put the resource files/folders/license at the required location(s). You can follow these steps to accomplish the same:
...
2. Unpack your editor's source code into the `editors` folder. For example, for Ketcher2 (version 2.18.0), you can run the following commands to download and extract the source code.
...
5. Go to [Admin Interface > UI features](../admin/features) and enable the editor. Please ensure that the UI-Features' JSON configuration is correct. Here's what is might look like:
