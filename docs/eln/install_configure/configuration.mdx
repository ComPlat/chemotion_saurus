...
It is possible to access remote desktops with VNC using websockify. A VNC server needs to be [installed](https://github.com/novnc/websockify) on each remote machine. Settings that need to be done are described in [Admin Interface > Remote Devices](../admin/remote_devices).
