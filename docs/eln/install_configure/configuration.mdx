...
This feature requires authorized access. As a system administrator, you can grant or deny users access to the function. Please see [Admin Interface > UI features](../admin/features)
...
Chemotion ELN uses Ketcher as the default structure editor. To use other editors, configure the required files and the license (if needed) in the `structure_editors.yml` file.
...
The attachments are then distributed to Chemotion ELN user by matching the additional addressee present in the **To:** or **CC:** field of the e-mail. The sender's e-mail address should also be one of an Chemotion ELN-registered device.

:::danger
All new incoming e-mails are processed and discarded.
If the sender and other addressees are unknown to the Chemotion ELN (e-mails registered to Chemotion ELN accounts), then the e-mail will be directly deleted.
:::
...
Setting: through the [Admin Interface > noVNC settings](../admin/novnc_settings)
