...
Please follow the configuration steps (see [here](install_configure/manual_install#configuring-the-installation) and [here](install_configure/configuration) to personalize your installation and then perform a [system check](install_configure/system_check) to make sure that all features are working as intended.
...
# Fixing Sample or Molecule SVG images (chemotion_ELN v1.5.4 or above)

A script to reprocess the SVGs of molecules and samples is available (see [PR1308](https://github.com/ComPlat/chemotion_ELN/pull/1308/commits/868477dc91cec99b689db2ac93106d04ce8dd694)).For this to run properly, the Ketcher back-end service has to be working. Note that depending on the situation, this could consume a lot of resources.
...
If docker compose is used, the command can be run  from the host where the docker-compose file is located. For an example using the worker service  of the docker compose file:
