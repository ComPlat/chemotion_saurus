...
In the element bar, samples are listed first as the most important common unit of this ELN. The tab of the samples has the benzene ring <img src={"/chemotionsaurus/img/benzene.png"} width={"30px"} alt={"Benzene"}/>  as a symbol. The user will notice the number next to the sample symbol and the number in parentheses. The number right next to the sample symbol indicates the number of samples within the selected collection. The number in parentheses shows the number of selected samples within the collection. The element bar shown in Figure 1 appears when the ELN is opened if no detail level has been selected. When selecting the level of detail by selecting a sample, the element bar is reduced and the [details modal](https://eln.chemotion.net/chemotionsaurus/docs/eln/details_modal#details-modal-for-samples) appears. 
