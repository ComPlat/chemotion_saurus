...
To configure the Ketcher backend render service differently, simply create a new configuration file called `ketcher_service.yml` in your config folder (by default that is `./shared/config`) with the following content:
...
```
mkdir -p ./shared/config/                   # this makes sure a configuration directory for the ELN exists
mkdir -p ./services/converter/              # this creates a directory holding service configurations
docker run --entrypoint htpasswd httpd:2 -sbn mysuser mypassword > ./services/converter/htpasswd   # this creates a user in the service's authentication database

cat >> ./shared/config <<EOF
production:
  :url: 'http://converter/'
  :profile: MYUSER
  :secret_key: MYPASS
