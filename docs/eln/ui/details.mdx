...
The hierarchy of analytical data is structured as follows. One or more `analyses` can be associated with a sample. Each analysis can contain one or more `datasets`. The latter can contain one or more `attachments`.
...
You can change the ordering of the analyses by drag and drop by clicking the <Btn mixed={[faEdit, "Edit mode"]} color={"primary"} /> button at the top of the analyses
tab. It will then switch to the `Order mode`. With the help of drag-and-drop functionality, you can now easily rearrange the
...
This panel includes 5 tabs named `Scheme`, `Properties`, `Analyses`, `References` and `Green Chemistry`.
