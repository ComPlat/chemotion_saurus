...
Wellplates can be created through different options via the dropdown menu in the <img src={require('@site/static/img/action_bar_create_wellplate.png').default} alt={"create wellplate"} height={"7%"} width={"7%"}/> in the <b>Action Bar</b>. Find the Action Bar in the structure of the [Action Bar](../../ui#action-bar).

The first element in the blue button looks differently according to the tab that is selected in the <b>Element Bar</b>, for example if you are on the Sample Tab there will be a <Sample title="Sample" className="logo" width={"30px"}/> instead of <Wellplate title="Wellplate" className="logo" width={"30px"}/>.
