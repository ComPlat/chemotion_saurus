...
import Reaction from "@site/static/img/eln/ui/elements/reactions/reaction.svg";
...
[Samples](/docs/eln/ui/elements/samples) are always an essential part of reactions.
...
If users want to assign another name to the reaction, they can do this by making an entry in the **Name** field within the **Scheme** tab or **Properties** tab.

## Generation

Reactions are generated with the green <Btn mixed={[faPlus, "CREATE"]} color={"success"}/> button in the [toolbar](../toolbar) by choosing **Create Reaction**.
...
## Reactions in the element panel

<img
  src={require("@site/static/img/eln/ui/elements/reactions/reactions_list.png").default}
  alt={"Reactions list"}
/>

In the element panel, the reaction tab has the symbol <Reaction title="Reaction" className="logo" width={"30px"}/>.
...
The user can filter their reaction either by creation date, or update date.
The image above shows a calendar icon <img src={require('@site/static/img/eln/ui/elements/reactions/creation_date.png').default} width={"30px"} alt={"Creation date calendar"}/>
...
and here the calendar appears like this <img src={require('@site/static/img/eln/ui/elements/reactions/update_date.png').default} width={"30px"} alt={"Update date calendar"}/>.
...
<img
  src={require("@site/static/img/eln/ui/elements/reactions/gas_scheme.png").default}
  alt={"Reactions list"}
/>
...
<video controls>
  <source src={require("@site/static/img/eln/ui/elements/reactions/weight_percentage_scheme.webm").default} />
</video>
...
- Reference product: When weight-percentage mode is active the material sample that serves as the weight reference must be selected. The target amount of the reference material defines the combined amounts of the other materials which have wt% values applied to them.

### How it works (step-by-step)

1. **Activation of weight percentage mode**
  - The user switches the scheme into Weight Percentage mode using the mode toggle. This enables the weight-percentage controls.
...
3. **Weight percentage fields**
  - As a result, the weight percentage (wt%) input field can be changed for starting materials and reactants.
...
- Amounts get recalculated automatically using the formula above when wt% or the reference target mass change.
