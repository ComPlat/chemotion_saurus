...
import Sample from "@site/static/img/eln/ui/elements/samples/sample.svg";
import Reaction from "@site/static/img/eln/ui/elements/samples/reaction.svg";
...
Samples are numbered consecutively and displayed in the list of samples according to their last use (this differs from the display of [reactions](/docs/eln/ui/elements/reactions)!). The sample number is preceded by the abbreviation of the Chemotion ELN owner so that samples can be clearly identified. If users want to assign a name (e.g. a trivial name) to a sample, they can do this by making an entry in the Name or Description field within the [Properties tab](../details#properties-tab). Please note that the Name field for samples from reactions is overwritten by additional information. A created sample will have the selected name, but when a new sample is created by dragging and dropping it into a [reaction scheme](../details#scheme-tab), the Name field will be overwritten. To save a second or more names for a sample permanently, use the **Description** field.
...
- [Importing](/docs/eln/ui/import) it from an external source.

### Generation of samples by drawing

The fastest and easiest method of generating a sample at the beginning of Chemotion ELN use is to create a new one by drawing a molecular structure in [Ketcher editor](../ketcher). Please note that samples can only be created if a target [collection](../collections) has been selected beforehand. First select the collection to which the sample should belong in the collection bar. After selecting a collection, use the green <Btn mixed={[faPlus, "CREATE"]} color={"success"}/> button in the [toolbar](../toolbar) and choose **Create Sample**.
...
You can find instructions on how to generate samples by drawing molecules in Ketcher editor [here](../ketcher), and you can learn about adding details on your sample [here](../details#details-panel-for-samples).
...
A generation of new samples can be carried out by two similar actions, **Split Sample** and **Copy Sample **, which will be depicted in the following.

The **Copy** action is available via the toolbar, or can be used within the element panel when dealing with reactions. To copy a sample, first select the sample whose structure is to be copied from the Samples list and then use the **Copy Sample** function from the toolbar. When the function is selected, a details panel is opened for a new sample to be edited.
...
To export sample data, navigate to the toolbar and click the dropdown export menu <img src={require('@site/static/img/eln/ui/elements/samples/dropdown_menu.png').default} width={"50px"} alt={"dropdown menu"}/>. A panel **Select Data to Export** will open up when **Export samples from selection** in the dropdown export menu is clicked.
...
<center>
  <img src={require("@site/static/img/eln/ui/elements/samples/export_menu.png").default} style={{ marginBottom: "30px" }}/>
</center>


:::danger[Caution]
The sample tab of the element panel doesn’t contain a list of samples generated as **Reactants** from within a reaction. Only samples that have been created as **Starting material** or as a **Product** are listed.
:::
