...
Collections that were shared with you can also be chosen as a target, grouped under
**Shared by \<owner\>**, provided you hold at least the *Add elements* permission on them.

:::caution

**Move to Collection** is not available while you are in the **All** collection. **All**
gathers everything you own rather than being a working collection, so there is nothing to
move the elements out of — use one of your own collections instead. See
[System collections](collections/index.mdx#system-collections).
...
`Remove from all Collections`: This action removes the selected elements from **all of your own collections**, and permanently deletes only those elements that are left in no collection at all. It never reaches into a collection owned by someone else, including one that has been shared with you — those keep their copy, and its owner goes on seeing the element. The user is asked to confirm their decision via a pop-up window by pressing <Btn mixed={["Delete"]} color={"warning"}/>, or otherwise to press <Btn mixed={["Cancel"]} color={"primary"}/>. An option is provided in case the user is trying to remove reactions, which is to choose whether to delete the samples associated with the reactions, if the user chooses not to, then only the reactants and solvents associated with the deleted reaction are deleted too.

## Sharing collections

Collections can be shared with individual users and with groups, at a permission level you
choose. To share the collection you are currently in, press
<Btn mixed={[faShareAlt]} color={"info"}/>; to share just a selection of elements as a new
collection, select them and choose **Share** from the selection actions.

The permission levels, the detail levels, and everything else about the sharing model are
described in [Sharing Collections](collections/sharing.mdx).
