...
The Location field is devided into 2 subsections `host location, host group ` which reflects information about owner of chemical and `current location, current group` in case of chemical is borrowed by another group/person. Both subsections have overlapping fields.
* `building, room, cabinet` where the chemicalis stored.
