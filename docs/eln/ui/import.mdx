...
This is the simple import method. For the enhanced, validated import introduced in version 3.0, see [Importing Data with Column Mapping and Validation](#importing-data-with-column-mapping-and-validation) below.

[Samples](/docs/eln/ui/elements/samples) can be imported to Chemotion ELN by uploading a simple excel list that contains the structure of the sample as a SMILES string.
Please add the SMILES strings in the respective column of the file to be imported and add the description "canonical smiles" as the header of the column.
To enable the import into a [collection](/docs/eln/ui/collections), the desired target collection has to be selected. Import of samples is only possible for the user that created the collection.
...
![Importing samples from an Excel file](/img/common/import_samples.gif)
...
:::info
This feature is available starting from version 3.0.
:::
...
1. Navigate to the desired collection in Chemotion ELN and click on the respective toolbar icon to open a list of options as shown below.
...
3. Drag and drop your file into the upload area or select it manually. Supported file formats include:
   ...
   - SDF (`.sdf`)
   - CSV (`.csv`)
4. The user can download any of the templates (available in `.xlsx` and `.sdf` format) by clicking on the **Download Template** button.
...
![enhanced sample file import](/img/eln/ui/import/sample_enhanced_import_1.gif)

---

## Step 2: Column Mapping

The column mapping interface allows users to match the columns in a file to the database fields in Chemotion ELN. This step ensures that the imported data is correctly aligned with the database schema.
...
![enhanced sample import column mapping](/img/eln/ui/import/sample_import_columns_mapping.gif)
...
1. **Schema Validation**: fields in the data are validated against predefined schema rules. The schema defines the expected data type, format, and constraints.
...
2. Correct any errors highlighted in the grid. Errors are displayed with detailed messages to guide the user.
3. Add or delete rows as needed.
4. Click **Validate** to ensure all data is valid.

![enhanced sample import data validation](/img/eln/ui/import/enhance_sample_import_data_validation.gif)
...
Once the data is validated, users can proceed with the import.

### Small Files
- For smaller files, the data is imported directly into the database.
- The user will see a confirmation message once the import is complete.

### Large Files
- For larger files, the import is handled as a backend job.
- This allows users to continue using Chemotion ELN while the import is processed in the background.
- The user will receive a notification once the import is complete.

---

## Additional Notes

- **Backend Job Handling**: The backend job ensures that large imports do not disrupt the user experience. The system will notify the user when the import is complete.
