...
1. Navigate to the desired collection in Chemotion ELN.
...
- **Smart Mapping**: Automatically suggests mappings based on column names and database fields.
- **Manual Mapping**: Select the appropriate database field for each column using dropdown menus.

### How to Use
1. Review the suggested mappings provided by the **Smart Mapping** feature.
...
1. **Schema Validation**: Each field in the data is validated against a predefined schema. The schema defines the expected data type, format, and constraints for each field.
...
---
#### Validation Process
The validation process ensures that the data conforms to the expected structure and meets specific field requirements. This functionality provides a unified validation
...
- **Validation**: The value must be a can be empty string.
