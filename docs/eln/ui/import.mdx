...
[Samples](elements/samples) can be imported to Chemotion ELN by uploading a simple excel list that contains the structure of the sample as a SMILES string.
...
To enable the import into a [collection](collections), the desired target collection has to be selected. Import of samples is only possible for the user that created the collection.
