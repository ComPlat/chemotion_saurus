...
![adminui-usermanagement](@site/static/img/eln/admin/user_management/user_management.png)

An ELN Admin can do the following:
...
- limit the user's disk usage

## Create user accounts

Press the following to open the dialogue window:
![adminui-usermanagement-createuser](@site/static/img/eln/admin/user_management/newuser.png)

Add the information for the new user account or upload a csv file containing the accounts' information.

![adminui-usermanagement-createuser-modal](@site/static/img/eln/admin/user_management/newuser-mod.png)

## Editing or deleting an existing account
...
![adminui-usermanagement-editing-modal](@site/static/img/eln/admin/user_management/editing.png)

![adminui-usermanagement-editing-modal](@site/static/img/eln/admin/user_management/deleting-mod.png)

## (De-)activating vs (Un-)locking an account

![adminui-usermanagement-lock-activate](@site/static/img/eln/admin/user_management/lock-activate.png)

A user account is locked automatically after too many failed login attempts (5 successive failed attempts).
The account will be automatically unlocked after 10 minutes.
...
Every user can create personal templates without any special permission. To allow someone to also create or edit global templates, the administrator needs to grant them the `Global Text Template Editor` privilege from this page.

![global-template-editor-button](@site/static/img/eln/admin/text_template/global_template_editor_button.png)
...
The user can see their quota in **Account & Profile**.
...
![adminui-usermanagement-default-user-space](@site/static/img/eln/admin/user_management/default_user_space.png)
...
their quota will be updated automatically if they meet these criteria.
:::
