...
The administrator can manage the features that require authorized access through the **UI features**. Two actions are available for each feature: **Edit Permission** <FontAwesomeIcon icon={faPenToSquare} /> and **Edit JSON** <FontAwesomeIcon icon={faGear} />. Sometimes, optional features (such as [sample in Decoupled mode](../ui/details#decoupled-mode)) are also included here, leaving it to the administrator to enable/disable such features.

![adminui-uifeatures](@site/static/img/eln/admin/features/features.png)
...
![adminui-uifeatures-edit_permission](@site/static/img/eln/admin/features/permission.gif)
...
This feature allows administrators to enable or disable the `FastInput` creation of [samples](/docs/eln/ui/elements/samples) using CAS numbers or SMILES strings.
...
To enable CAS Common Chemistry API, add the API key in the configuration as shown below:
...
This feature allows administrators to configure [third-party authentication methods](/docs/eln/install-configure/authentication) for user login, such as OpenID Connect (OIDC) and Shibboleth.
...
| scheme                 | 'https'                                                        |
| client_id              | client ID                                                      |
| redirect_uri           | the Chemotion ELN URL + '/users/auth/openid_connect/callback' |
| client_secret          | secret                                                         |
| token_endpoint         | '/oauth2/token'                                                |
| authorization_endpoint | '/oauth2/auth'                                                 |
