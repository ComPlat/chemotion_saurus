...
The administrator can manage the features that require authorized access through the **UI features**. Two actions are available for each feature: **Edit Permission** <FontAwesomeIcon icon={faPenToSquare} /> and **Edit JSON** <FontAwesomeIcon icon={faGear} />. Sometimes, beta features (such as [sample in Decoupled mode](../ui/details#decoupled-mode)) are also trialed here, leaving it onto the administrator to enable/disable such features.
