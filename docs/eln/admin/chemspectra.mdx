...
In ChemSpectra, a **layout** refers to a spectrum type. So when a data file is sent to [ChemSpectra](../../services/chemspectra), the data type is mapped to its corresponding layout and processed accordingly.

If a data file is from a supported technique and extension but isn't processed by ChemSpectra as expected, it may be due to an unrecognized data type. Administrators can seamlessly update the data types list through the [ELN admin](.).
