...
The __Disk Usage__ column shows how much storage a user is allowed to use
and also shows the __percentage of allocated storage used__, if a limit has been set.
This limit helps manage the amount of space used by uploaded attachments and generated reports.
It can be set __when creating or updating a user__.
Storage usage is calculated __once per day__ by adding up the total size of:
...
The user can see there quota in __Account & Profile__.
...
In the dashboard the __default user allocated space__ can be set 
by entering a value and clicking __Save__. This value will appear in the __User Management__ tab
and is automatically assigned to all __new users__.

:::info[Info]
If both a default value and individual user quotas are set,
only users who have __no set quota__ or a __lower quota__ will be updated.
:::

:::warning[Important]
You cannot reduce a user's existing quota. It can only be __increased__.
:::

:::tip[Groups]
Setting __Disk Usage__ quota for a group will apply that value to all users in the group
who have __no set quota__ or a __lower quota__. If a user is assigned to such a group,
