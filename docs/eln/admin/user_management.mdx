...
![adminui-usermanagement](@site/static/img/admin_ui/user_management/user_management.png)
...
![adminui-usermanagement-createuser](@site/static/img/admin_ui/user_management/newuser.png)

Add the information for the new user account or upload a csv file containing the accounts' information.

![adminui-usermanagement-createuser-modal](@site/static/img/admin_ui/user_management/newuser-mod.png)
...
![adminui-usermanagement-editing-modal](@site/static/img/admin_ui/user_management/editing.png)

![adminui-usermanagement-editing-modal](@site/static/img/admin_ui/user_management/deleting-mod.png)

## (De-)activating vs (Un-)locking an account

![adminui-usermanagement-lock-activate](@site/static/img/admin_ui/user_management/lock-activate.png)
...
Everyone can see the **Global** tab and browse global templates through Text Templates page. This privilege only unlocks the ability to add, edit, or delete them.

## Affiliation Approval Rights

Reviewing [affiliation suggestions](/docs/eln/admin/affiliations) doesn't have to be an admin-only task. Click the university icon in a user's **Actions** column to grant them the `Affiliation Moderator` privilege.

![affiliation-moderator-button](@site/static/img/admin_ui/user_management/affiliation_moderator_button.png)

A user with this privilege sees a **Requests** tab next to **My affiliations** in their own **Settings → Affiliations**, where they can edit, approve, and reject pending suggestions from any user — the same tools available on the [admin Affiliations page](/docs/eln/admin/affiliations). They do not gain access to the admin panel itself; this privilege only unlocks that one review screen inside their normal settings.
...
![adminui-usermanagement-default-user-space](@site/static/img/admin_ui/user_management/default_user_space.png)
...
Updating the default value will also only update these users.
:::

:::tip[Groups]
Setting or updating **Disk Usage** quota for a group will apply that value to all users in the group
