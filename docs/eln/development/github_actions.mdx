...
#### Acceptance tests
##### Linting
Before the acceptance tests are running, changed JavaScript or Ruby files will be linted. If no changed files are pushed, `git add $FILES_JS $FILES_RB` will throw an error: `Error: Process completed with exit code 1.`, because there are no files to add. But the workflow for acceptance tests will continue because of `continue-on-error: true` in the workflow.  
