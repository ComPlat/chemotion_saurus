...
The first testing is done using the `Mocha` framework: https://mochajs.org/.
...
## 2. Ruby unit tests
Another testing is done with the `RSpec` framework: https://rspec.info/.
...
For locally testing the Chemotion ELN, some changes to the root directory of your Chemotion ELN code is necessary. For this type the following commands: 
