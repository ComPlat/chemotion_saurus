...
skip all
``` 
In the file with the acceptance tests is some [linting](#linting) included. Linting can be skipped with this command in the commit message:
```sh 
skip linting 
```
### User Interface
...
#### Authentication
##### Token
Some operations need a way to authenticate during a workflow run. GitHub provides an automatic token, which is only valid during a workflow run: [GITHUB_TOKEN](https://docs.github.com/en/actions/reference/authentication-in-a-workflow). In the workflow the token can be accessed like this:
```sh 
${{ github.token }}
${{ secrets.GITHUB_TOKEN }}
```
##### GitHub bot
Another way to authenticate is using the bot user provided by GitHub, e. g. with this action: [setup-git-user action](https://github.com/fregante/setup-git-user).
This is useful if you want to push to a repository inside a workflow. The GitHub bot prevents recursive workflows, that means an automatic push from it does not invoke the normal CI workflows for the repsoitory.
If you can not use the setup-git-user action (because you are using another user than root etc.), you can configure the GitHub bot user in the workflow with these credentials:
```sh
...
and this [GitHub issue in the rails/webpacker repository](https://github.com/rails/webpacker/issues/2654). This article describes a way to customize the devtool setting: [How to speed up assets precompile for Ruby on Rails apps](https://itnext.io/how-to-speed-up-assets-precompile-for-ruby-on-rails-apps-e0338d8d7301). The devtool controls how error paths are resolved in the browser console. In test and production mode no error log is needed, therefore the devtool can be set to the fastest configuration. 
...
#### PostgreSQL
For the database a [PostgreSQL container](https://docs.github.com/en/actions/guides/creating-postgresql-service-containers) is used. The databases and users must be extra created.
...
::: 
Before the acceptance tests are running, changed JavaScript or Ruby files will be linted. The changed files will be automatically pushed to the repository via the [GitHub Bot](#github-bot).
If no files are changed, `git add $FILES_JS $FILES_RB` will throw an `Error: Process completed with exit code 1.`, because there are no files to add. But the workflow for acceptance tests will continue because of `continue-on-error: true` in the workflow.  
