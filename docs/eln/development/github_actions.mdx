...
GitHub Actions are the Continuous Integration and Deployment tool of GitHub. They are used for testing newly created code in development and activated on every push to the [ELN GitHub repository](https://github.com/ComPlat/chemotion_ELN). Every test class (JavaScript, Ruby and acceptance) has its own workflow.
