...
Sharing an _explicit_ git-workflow is essential for efficient collaboration, especially for new team members.
...
They represent the flow of information in the sense of
...
Make sure that your branching `feature` off of the up-to-date `main` branch.
...
That means that we're integrating changes by **rebasing** in order to maintain a clean git history.
...
During rebasing you might have to [resolve merge conflict](TODO_link_to_resource).
After rebasing, you might have to [force push](https://www.git-tower.com/blog/force-push-in-git/)
...
Writing clear and concise commit messages is very helpful for reviewing a feature branch and searching the git history.
...
Keep evolving your feature branch publicly, rather than committing locally and then pushing only "when the feature is done".
Facilitates collaboration, as well as early, focussed review and feedback.
...
* how to merge PR? rebase `feature` onto `main` with squashed commits (full history preserved in PRs)

## Misc

* proposal to rename "development-5" to "main" (or "development")
* if the changes are visible to the end-user, video or screenshots should be added to the PR, in order to facilitate user testing
* new functionality is covered with tests / no drops in test coverage
* add a bullet point on top of the CHANGELOG (optional: reference to github issue/PR)
