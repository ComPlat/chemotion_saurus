...
Containers enable the use of isolated, consistent, and portable development environments. When using containers for development you will not have to install and configure the Chemotion ELN locally on your machine.
...
Have a look at this [tutorial](https://carpentries-incubator.github.io/docker-introduction/introduction/index.html) for an accessible introduction to containers.

## Prerequisites

This guide assumes you are familiar with basic operation of Git, Docker and a Linux operating system.
At minimum, your hardware setup should feature 3 GB RAM and 20 GB disk capacity (it is advisable to assign considerably more resources).

:::caution
We recommend developing the ELN in a Visual Studio Code devcontainer
:::

## Dependencies ane Set up
...
Once you have `WSL 2` and `Docker Desktop` installed, you can start setting up the Chemotion ELN development environment.
The development environment consists of the Chemotion ELN source code as well as two Docker images containing all dependencies and configurations of the Chemotion ELN runtime.
...
If you get a response you're now ready to run the ELN in a [Docker container](#eln-in-docker) or in a [Visual Studio Code devcontainer](#eln-in-devcontainer).
...
- Visual Studio Code: You need install the VS Code Docker extension. This extension adds the functionality to build, manage, and deploy containerized applications from inside VS Code. (You need the Remote-Container extension to actually use the container as your dev environment.)

### Set up

- Run Docker
* Create a folder where you want to keep Dockerfiles and shell scripts for dockerization
* Open the terminal in the previous folder and clone the following [repository](https://github.com/ptrxyz/chemotion/tree/chemotion-cli/client-vscode-remote-container) to it:

```
git clone https://github.com/ptrxyz/chemotion.git
```

- Switch to `v1.0.3D0.1` branch:

```
git checkout v1.0.3D0.1
```

:::info
There was a pull request when this documentation was last updated and it is provided by this link: <https://github.com/ptrxyz/chemotion/pull/19>. Make sure that you have those fixes if you install chemotion before it is merged.

:::

* change into the `client-vscode-remote-container` folder:
...
* Please note that compiling "webpacker" takes a little time, so wait a little for it, and then start the server:
...
You are ready now. You can find more details in this [link](https://github.com/ptrxyz/chemotion/tree/chemotion-cli/client-vscode-remote-container) and this [video](https://www.youtube.com/watch?v=HZCAbC6ldzE).
