...
To link this `proxy` network with the ELN's network, we need to include it as a key-value pair for the `eln` service i.e. in the `docker-compose.cli.yml` file, under the service called `eln`, we need to do the following:
