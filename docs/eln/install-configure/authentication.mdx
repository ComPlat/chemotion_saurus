...
import designBase from "@site/static/img/eln/install_configure/authentication/design-base.png";
...
The Chemotion team also offers third-party authentication options, including Shibboleth, OIDC (Open ID Connect), ORCID, GitHub, and NFDI AAI. To configure these options, visit the [Chemotion Administration](/docs/eln/admin) page. On the left side, under '[UI Features](/docs/eln/admin/features)', find the function labeled 'User Provider'. Click on the yellow 'Edit JSON' button to access the configuration settings.

![adminui-uifeatures-edit_permission](@site/static/img/eln/install-configure/user_provider.png)
...
```sh
{
  openid_connect: {
    ...
    redirect_uri: "https://chemotion.dept.uni.de/users/auth/openid_connect/callback",
...
In ELN 1.6, the Chemotion team introduced several third-party login authentication methods, including Shibboleth and OpenID Connect. Since NFDI AAI RegAPP uses OpenID Connect, it is ready for use with ELN version 1.6 or higher.
...
2. Your FQDN (Fully Qualified Domain Name) (e.g., https://chemotion.dept.uni.de)
3. ELN Version
