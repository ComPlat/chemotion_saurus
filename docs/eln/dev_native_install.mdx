...
```
This error may occure if you install chemotion_ELN for multiple users on the same machine, because global packages to be installed already exists, so either comment out the installation of the package which causes the problem or delete them.
...
Either the database name in the yml configuration file needs to be changed or the database needs to be created in Postgres.
