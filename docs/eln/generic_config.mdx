...
| formula field | v0.9.0+ |---| a calculation field; as an admin, define a formula using other fields, which are supposed to be either "integer" or "System-Defined", and system do the calculation automatically. The result appears in the corresponding field, and it keeps getting updated instantly as long as the values used to calculate it are being updated. When the user finish updating the values, they could press **adjust** <Btn mixed={[faArrowRight]} color={"secondary"}/> to adapt the final result, and they could also edit the final result manually |
