...
| formula field | v0.9.0+ |---| a calculation field; as an admin, define a formula using other fields, and the system does the calculation automatically ([details](#field-type-formula-field)) |
...
<img src={"/chemotionsaurus/img/formula.gif"} alt={"Formula field"}/>

The field type **Formula-Field** enables automatic calculations based on a formula. The admin can use either "Integer" or "System-Defined" **Field Name**s as input values in the **Formula** field in the adminstration page. For instance, if there are two already defined **Integer** fields that are **a** and **b**, a possible definition of the formula for the field **c** can be **a+b**. For the user, the result of that addition appears in the corresponding field **c**, which appears in gray as it is uneditable, and it keeps getting updated instantly as long as the values used to calculate it are being updated. When the user finishes updating the values, they could press **adjust** <Btn mixed={[faArrowRight]} color={"secondary"}/> to adapt the final result, and they could also edit the final result manually, and give it values that don't correspond to the input values. This action doesn't affect the values used to calculate the result (**a** and **b**)
