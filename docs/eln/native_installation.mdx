...
```
3. [Allow remote connection to postgres](#allow-remote-connection-postgres)
  [You can do that by following below link](https://www.thegeekstuff.com/2014/02/enable-remote-postgresql-connection/)
...
1. Copy the installation script on a ubuntu server 18.04 or 20.04
```sh
...
Thia will basically install evrything on one machine but configuration within can be reworked to just serve one purpose.
The script will
     - [x] install OS package dependencies
     - [x] install passenger
     - [x] create a new user
     - [x] install ruby and nodejs for the user
     - [x] create a postgresql DB
     - [x] copy the chemotion_ELN code and prepare basic config files
     - [x] do a capistrano app deploy
     - [x] config nginx (NB: no ssl set) and UFW
...
5. Migrate the DB and restart the app
```sh
sudo su production
...
7. To uninstall Postgres 
  Stop the DB server first
  ```sh
  sudo service postgresql stop
  ```
 8. To stop worker job       *  
