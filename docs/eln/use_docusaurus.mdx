...
## Versioning
Currently versioning is not enabled for the Chemotion ELN part of the documentation, but this could be quickly changed in the future. To create a new documentation version based on the latest content in the ```docs/eln``` directory follow these steps:
...
1. in ```sidebarELN.js```: Add the new version to the ```Versions``` category.
