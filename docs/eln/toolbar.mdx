...
The user can filter a selected collection or even the **All** collection with the filtering bar <img src={"/chemotionsaurus/img/filter.png"} alt={"Filter bar"}/>. The user can search the entire collection or selected elements of it by clicking on <Btn mixed={["All", faSortDown]} color={"secondary"}/> and choosing the desired element (**Samples**, **Reactions**, **Wellplates**, **Screens**) from the menu to appear, where the <Btn mixed={["All", faSortDown]} color={"secondary"}/> is replaced by the corresponding symbol of the element. The menu also shows the option of **Advanced Search**, which will be clarified later on in this chapter. 
