...
The **Archive** tab contains all the generated reports (except the deleted ones). Here the user can reuse previous reports to generate new ones using <Btn mixed={[faPen] color={"caution"}}/>. They can download the generated reports with <Btn mixed={[faDownload] color={"primary"}}/>, or delete them with <Btn mixed={[faTimes]} color={"danger"}/>.
