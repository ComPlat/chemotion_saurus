...
The procedure is split into two aspects that can be set up independently or in parrallel:
...
Create a new file in ChemStation's main program folder: *\Chem32\CORE\ appropialtely named (example: ELN_Export.mac"), make sure the file is of the *.mac file type.
...
In ChemStation load the method that shoud transfer data.
...
Activate the "Post-Run Command / Macro:" check box and enter the command to rund the macro (exampe: "Macro "ELN_Export.mac", go")
