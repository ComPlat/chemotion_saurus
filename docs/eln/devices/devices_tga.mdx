...
In this guideline, we have explained the integration process of the TGA device into the Chemotion ELN. This procedure
...
For the TGA device integration, we have decided to configure both these features.
...
### Configurations in TGA PC
The TGA PC and the Chemotion ELN both need access to a common network storage location. This can be set up on a
third-party storage (a NAS server, shared storage of another PC etc.) or it can be set up locally on the TGA PC. In this
...
The Chemotion ELN’s data collector has to be configured to collect data of the TGA device from the network location.
