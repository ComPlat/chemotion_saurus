...
For the NMR integration, the Chemotion team decided to configure both these features.
...
Spinsolve's data storage options show that the data is stored both locally and on a remote server.
For the remote backup storage location, the previously created network drive is used. The Chemotion ELN also has
...
![devices_NMR_magritek_10](/img/eln/devices/nmr/magritek_spinsolve/devices_nmr_magritek_10.png)

To make sure that Chemotion ELN is able to transfer it to the appropriate user's inbox, the user has to make sure the samples are named following the [naming convention](../../devices#name-conventions).
...
![devices_NMR_magritek_11](/img/eln/devices/nmr/magritek_spinsolve/devices_nmr_magritek_11.png)
