...
![ShuttleBuilder](/img/eln/devices/configurations/data_transfer/shuttle_builder.drawio.png)
...
data storage and allows monitoring and organizing the data transmission of devices integrated with the user's ELN Chemotion instance.
Additionally, it can process data in a configurable, step-based workflow.
...
The Chemotion team strongly recommends using the ShuttleBuilder, as it reduces configuration errors and simplifies setup.
Nevertheless, all available configuration options are documented below. The same arguments can be provided either via a .env file or as CLI arguments.
  ...
  - **preconvert** [string]\
    	...
    	- The user must specify two destinations
...
The ShuttleBuilder is a service that allows users to create and manage their Shuttle instances. It operates independently of the Electronic Lab Notebook (ELN) Chemotion.
...
Users can set up their own ShuttleBuilder instance.
The Chemotion team provides the Docker Compose file below, which includes everything needed to host the ShuttleBuilder.

Make sure to set the variables correctly. The .env file below the Docker Compose file lists all required variables.
...
- To connect it to a Chemotion instance, set ELN_URL to the correct URL.
