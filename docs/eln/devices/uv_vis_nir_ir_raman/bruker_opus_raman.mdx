...
The EFW program will run on the device's computer in the background and transfer the *.0 and *.dx data files from the local data storage to the network location accessible by both the device's computer and the ELN.
