...
describe 'reaction API' do
  ...
  let(:sample_2) do
    create(
      :sample, name: 'Sample 2', container: FactoryBot.create(:container)
    )
  end
  let(:sample_3) do
    create(
      :sample, name: 'Sample 3', container: FactoryBot.create(:container)
    )
  end
  let(:sample_4) do
    create(
      :sample, name: 'Sample 4', container: FactoryBot.create(:container)
    )
  end
...
RSpec.configure do |config|
  config.around do |example|
    if example.metadata[:profile]
      profile = StackProf.run(mode: :wall, interval: 250000, raw: true, out: 'tmp/stackprof.dump') do
        example.run
      end
    else
      example.run
