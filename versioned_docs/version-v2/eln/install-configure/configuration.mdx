...
[Helpdesk](https://chemotion.net/helpdesk). This is especially true if you are using a non-dockerized installation of the ELN.
...
From Chemotion ELN v3 onwards, [EPAM Ketcher](https://lifescience.opensource.epam.com/ketcher/) will be the default structure editor. Do not forget to enable it as a feature from the ELN adminstration interface.

If you wish to utilize other supported structure editors, such as Marvin JS, additional configuration is required. In such cases, you'll need to adjust the necessary files and, potentially, provide a license. Please be aware that any modifications to the structure_editors.yml file will necessitate a service restart for the changes to take effect.
...
So, if you are using e.g. ELN version 3.0.0, please `nmriumwrapper.url` to `https://nmrium.nmrxiv.org/v1.0.0/`.

To be able to access these URLs successfully, you must register with the hosting service. Please contact the [Helpdesk](https://chemotion.net/helpdesk) or see [here](https://github.com/NFDI4Chem/nmrium-react-wrapper#public-instance) for more details.
