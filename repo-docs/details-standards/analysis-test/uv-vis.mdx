---
title: UV/Vis Analysis
sidebar_label: UV/Vis
author: Nicole Jung
sidebar_position: 5
---

:::info[Important information for this page]

This page contains a collection of ideas to give inline notations for analytical data in the future. We provide this page to discuss options for a clear formatting of descriptions - but there is no need to adapt to the given rules at the moment. This is a test area. Nevertheless: if you have suggestions and comments: please let us know or provide your ideas via the issue tracking option.
https://github.com/ComPlat/chemotion_saurus/issues

:::

Example for analysis of quantitative UV-Vis: <br />
UV-VIS (abs, CH<sub>3</sub>OH, 10 mM, 20 °C); λ<sub>max</sub> [nm] (log ε, M<sup>-1</sup>cm<sup>-1</sup>) = xxx (a.aa) [sh], yyy (b.bb) [sh].<br />

Example for analysis of qualitative UV-Vis: <br />
UV-VIS (abs, CH<sub>3</sub>OH, 10 mM, 20 °C); λ<sub>max</sub> [nm] = xxx [sh], yyy [sh].<br />

<b>Explanation:</b>
xxx, yyy -> λ<sub>max</sub> in nm <br />
(a.aa),(b.bb) -> log ε in M<sup>-1</sup>cm<sup>-1</sup> <br />
[sh] -> details if there are any, e.g. assignment
