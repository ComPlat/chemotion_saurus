...
DOIs are generated automatically in the repository - there is no need for action. Users just need to know where to find DOIs and how to use them. The DOIs are created (reserved) directly when the user [submits](workflow/submit) a reaction or sample. After the information in the submission panel is added, the DOIs are created. At this stage, the DOIs can be copied and used for the referencing in SIs and publications, but the DOIs don't resolve yet. Therefore, the data is not accessible to others. The DOIs will resolve at the time the dataset is released and published.
