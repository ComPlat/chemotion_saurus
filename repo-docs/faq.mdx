...
- **Answer:** The [embargo](/docs/repo/workflow/embargo) can only be released by the submitter of the datasets. Only if all data that belong to the embargo bundle are complete and reviewed, the release is possible. Please see also the Usage Pages for further information.
...
- **Answer:** The [Chemotion-Repository](/docs/repo) assigns a [DOI](/docs/repo/doi) for each dataset (and molecules, [reactions](/docs/eln/ui/elements/reactions)) via DataCite. The name used in the Publisher namespace is “Chemotion.net”. The Chemotion-Repository is assigned to the DOI string 10.14272.
...
- **Answer:** Metadata support is provided for information like analysis types and reaction names. The metadata can be selected via dropdown menus (supports search) that provides the embedded [ontobee](https://ontobee.org/) ontologies CHEMINF and CHMO. The Chemotion team uses the metadata format of DataCite for DOI submissions.
...
- **Answer:** Hints and demos for the repository and the [ELN](/docs/eln/ui) can be found on this [YouTube video](https://www.youtube.com/channel/UCWBwk4ZSXwmDzFo_ZieBcAw).

#### Has the repository undergone WDS (World Data Systems), DSA (Data Seal of Approval) or CTS (Core Trust Seal) accreditation?

- **Answer:** No, but the Chemotion team will apply for a Core Trust Seal soon.
...
- **Answer:** No limitation so far but the Chemotion team might limit it to 50 MB in the future.

#### Are there any limitations to the amount of data that an individual is able to upload?

- **Answer:** So far not. The Chemotion team will limit the size if misuse is detected. The data to be disclosed are reviewed and misuse will be detected.
...
- **Answer:** Reviewing first takes place by an automated quick check of most common data like NMR data (counting of signals necessary and analyzed) and in addition by a peer reviewing by the repository owners. Reviewing workflow includes comment functions for the datasets that allow data to be rejected and revisions to be requested (mediated via the repository UI and per email for notifications). If the data is submitted to the repo with embargo, the user has the option to provide access to the data to single external reviewers (access given by limited accounts, provided by mail).

#### How does the infrastructure of Chemotion repository look like?

![](/img/repo/faq/image_repo_infrastructure.png)
