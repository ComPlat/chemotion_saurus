...
- **Answer:** The [embargo](workflow/embargo) can only be released by the submitter of the datasets. Only if all data that belong to the embargo bundle are complete and reviewed, the release is possible. Please see also the Usage Pages for further information.
...
- **Answer:** The [Chemotion-Repository](.) assigns a [DOI](doi) for each dataset (and molecules, [reactions](/docs/eln/ui/elements/reactions)) via DataCite. The name used in the Publisher namespace is “Chemotion.net”. The Chemotion-Repository is assigned to the DOI string 10.14272.
